	...
	switch( skill_id ) {
		...
		case RK_HUNDREDSPEAR:
			...
			if( sd ) {
				...
				skillratio += 90 * pc_checkskill(sd,LK_SPIRALPIERCE);
...
#endif
			ATK_ADD(wd.damage, wd.damage2, 40*lv);
		}
	}
...
struct Damage battle_calc_weapon_final_atk_modifiers(struct Damage wd, struct block_list *src,struct block_list *target,uint16 skill_id,uint16 skill_lv)
{
	...
	if( sc ) {
		...
		if (sc->data[SC_FUSION]) {
			...
			if (sd && tsd) {
				hp = 1*hp/100;	//default: hp = 8*hp/100;
				if (((int64)sstatus->hp * 100) <= ((int64)sstatus->max_hp * 20))
					hp = sstatus->hp;
			} else
				hp = 1*hp/100; //2% hp loss per hit	//default: hp = 2*hp/100;
			status_zap(src, hp, 0);
		}
