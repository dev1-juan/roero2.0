...
int pc_gainexp(struct map_session_data *sd, struct block_list *src, unsigned int base_exp, unsigned int job_exp, bool quest)
{
	...
	if(sd->status.base_level == 255 && quest == 0)
	 base_exp = 0; // no base exp if base level is 255

	if(sd->status.base_level == 256 && quest == 0)
	 base_exp = 0; // no base exp if base level is 255

	if(sd->status.base_level == 257 && quest == 0)
	 base_exp = 0; // no base exp if base level is 255

	if(sd->status.base_level == 258 && quest == 0)
	 base_exp = 0; // no base exp if base level is 255

	if(sd->status.base_level == 259 && quest == 0)
	 base_exp = 0; // no base exp if base level is 255

	if(sd->status.base_level == 260 && quest == 0)
	 base_exp = 0; // no base exp if base level is 255

	if(sd->status.base_level == 261 && quest == 0)
	 base_exp = 0; // no base exp if base level is 255

	if(sd->status.base_level == 262 && quest == 0)
	 base_exp = 0; // no base exp if base level is 255

	if(sd->status.base_level == 263 && quest == 0)
	 base_exp = 0; // no base exp if base level is 255

	if(sd->status.base_level == 264 && quest == 0)
	 base_exp = 0; // no base exp if base level is 255
	...
	if(sd->status.guild_id>0)
		job_exp-=guild_payexp(sd,job_exp);
